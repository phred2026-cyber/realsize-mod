...
public class RealSizeMod implements ModInitializer {
    ...
    private double getScale(EntityType<?> type) {
        ...
        // Arthropods — dramatically smaller than vanilla, still visible
        if (type == EntityType.SPIDER)        return 0.30;
        if (type == EntityType.CAVE_SPIDER)   return 0.22;
        if (type == EntityType.BEE)           return 0.25;
        if (type == EntityType.SILVERFISH)    return 0.20;
        if (type == EntityType.ENDERMITE)     return 0.18;

        // Small animals
        if (type == EntityType.BAT)           return 0.30;
        ...
        if (type == EntityType.TADPOLE)       return 0.20;
